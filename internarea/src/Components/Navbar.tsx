    ...
    <div className="relative">
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out bg-white border-t border-gray-100 ${isMenuOpen ? 'max-h-[500px] py-4' : 'max-h-0'}`}>
          <div className="px-4 space-y-4 pb-4">
            <Link href="/internship" onClick={() => setIsMenuOpen(false)} className="block text-gray-700 font-medium py-2 border-b border-gray-50">{t("internships")}</Link>
            <Link href="/job" onClick={() => setIsMenuOpen(false)} className="block text-gray-700 font-medium py-2 border-b border-gray-50">{t("jobs")}</Link>
            
            <div className="flex items-center bg-gray-50 rounded-xl px-4 py-3 mb-4">
              <Search size={18} className="text-gray-400" />
              <input type="text" placeholder={t("search")} className="ml-3 bg-transparent focus:outline-none text-sm w-full" />
            </div>
            ...
            {user ? (
              ...
              <div className="space-y-4">
                <button 
                  onClick={handlelogin}
                  className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-100 py-3 rounded-xl font-bold text-sm hover:bg-gray-50 active:scale-95 transition-transform"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
